...
const Register = () => {
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      employeeId: Yup.string().when(["Select_Company", "colan"], {
        is: (company, colan) =>
          company === "Colan" && ["Fresher", "Experience"].includes(colan),
        then: () => Yup.string().required("Employee ID is required"),
        otherwise: () => Yup.string().notRequired(),
      }),
  ...
  return (
    <div className="flex flex-col w-full justify-start items-center gap-5">
      <div className="lg:pt-12 w-full max-w-md rounded-2xl ">
        <p className=" heading max-md:w-[40%] sm::relative bottom-5   max-md:border-b-2 pb-2 border-blue-400 max-md:text-lg text-2xl mb-4 font-semibold sm:text-start lg:text-center tracking-widest ">
          Sign up
        </p>
